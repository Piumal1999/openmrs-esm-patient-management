...
export const deleteCohortType = async (api: APIRequestContext, uuid: string) => {
  await api.delete(`rest/v1/cohortm/cohorttype/${uuid}?purge=true`, { data: {} });
};
...
export const editCohort = async (api: APIRequestContext, cohortUuid: string): Promise<Cohort> => {
  const cohortRes = await api.post(`rest/v1/cohortm/cohort/${cohortUuid}`, {
    data: {
      name: `Edited cohort ${Math.floor(Math.random() * 10000)}`,
      description: `Edited cohort description ${Math.floor(Math.random() * 10000)}`,
    },
  });
  await expect(cohortRes.ok()).toBeTruthy();
  return await cohortRes.json();
...
export const deleteCohort = async (api: APIRequestContext, uuid: string) => {
  await api.delete(`rest/v1/cohortm/cohort/${uuid}?purge=true`, {
    data: {
